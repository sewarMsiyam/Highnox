...
$('.selectpicker[multiple]').on('changed.bs.select', function (e) {
  ...
  if (selectedCount >= 3) {
    $(this).parent().find('.filter-option-inner-inner').html(`<span>${selectedCount} options selected</span>`);
  }
});
...
toggleSliderMenu[i].addEventListener("click", function () {
    toggleDisplay[i].classList.toggle("display-toggle-menu");
    document.querySelectorAll(".sidebar-arrow-down")[i].classList.toggle("fa-angle-down");
